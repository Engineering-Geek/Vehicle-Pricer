...
def fetch(page, addition='')->bs.BeautifulSoup:
    try:
...
def make_menu(all_makes)->pd.DataFrame:
...
def get_model_specification_links(df: pd.DataFrame)->pd.DataFrame:
...
def specs_and_pics(df)->pd.DataFrame:
...
def get_specs_pics()->pd.DataFrame:
